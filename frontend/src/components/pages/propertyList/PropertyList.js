...
import { getTypeLabel } from "../../../requests/requests";
import FilterModal from "../../ui/filterPopup/FilterModal";
...
const PropertyList = () => {
  ...
  const handleFilterApply = async (newFilters) => {
    try {
      ...
      const response = await axios.get(`/api/props?pageNumber=0`, {
        params: { ...newFilters, sortField: null, sortType: "ASC" },
      });
      const fetchedProperties = response.data.content;
      const totalPages = response.data.totalPages;
      ...
      <div className="container">
        ...
        <div className="filter-button">
          ...
          <div className="sort-section">
            ...
            <span
              className={`sort-link ${sortField === "price" ? "active" : ""}`}
              onClick={() => handleSortClick("price")}
            >
              Цене&nbsp;
              {sortField === "price" &&
                (sortType === "ASC" ? <FaSortUp /> : <FaSortDown />)}
            </span>
            <span
              className={`sort-link ${sortField === "rooms" ? "active" : ""}`}
              onClick={() => handleSortClick("rooms")}
            >
              Кол-ву комнат&nbsp;
              {sortField === "rooms" &&
                (sortType === "ASC" ? <FaSortUp /> : <FaSortDown />)}
            ...
            <span
              className={`sort-link ${sortField === "square" ? "active" : ""}`}
              onClick={() => handleSortClick("square")}
            >
              Площади&nbsp;
              {sortField === "square" &&
                (sortType === "ASC" ? <FaSortUp /> : <FaSortDown />)}
          ...
          {isFiltered && (
            <button
              ...
              onClick={() => {
                setIsFiltered(false); // Сброс значения isFiltered при клике на кнопку "Сбросить всё"
                  ...
                  <div className="card">
                    ...
                    <div className="card-body">
                      ...
                      <p className="card-text">
                        {getTypeLabel(property.type)}, {property.rooms} комн.,{" "}
                        {property.square} кв. м.
                      </p>
                    </div>
                  </div>
                </div>
